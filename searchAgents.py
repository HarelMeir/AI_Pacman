...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.startState, self.visited_corners
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                if new_position in self.corners:
                    i = self.corners.index(new_position)
                    corners_status[i] = True
                new_state = (new_position, corners_status)
...
def cornersHeuristic(state, problem):
    ...
    for i in range(4):
        if not state[1][i]:
            xy2 = corners[i]
            distances.append(abs(xy2[0] - xy1[0]) + abs(xy2[1] - xy1[1]))
    if distances:
